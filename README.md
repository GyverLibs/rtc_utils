...
- Одна единица `offset` - 4 байта памяти. Область должна помещаться целиком: `offset + rtc_size(&data) <= 128`
- Библиотека дописывает 4 байта crc в начало области, учитываем это при расчёте смещений для других данных
- Хранить можно только POD-данные без указателей и динамических ресурсов: базовые типы и простые структуры
- Размер данных автоматически дополняется до 4 байт при записи в RTC
